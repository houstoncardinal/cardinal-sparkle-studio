...
import {
  ...
  Plus, Download, Save, Users, Share2
} from 'lucide-react';

const Toolbar = () => {
  const { zoom, setZoom } = useDAWStore();
  ...
  const tools = [
    ...
    null,
    { icon: <Magnet size={13} />, label: 'Snap', active: true },
    null,
  ...
  return (
    <div className="h-8 bg-daw-surface border-b border-border flex items-center px-3 gap-1">
      ...
      <button className="p-1.5 rounded text-muted-foreground hover:text-foreground hover:bg-secondary transition-colors" title="Export">
        <Download size={13} />
      </button>
      <button className="p-1.5 rounded text-muted-foreground hover:text-foreground hover:bg-secondary transition-colors" title="Collaborate">
        <Users size={13} />
      </button>
